...
- Jobs invoke `build.py` without a `--filter`, so a `[filter]` table in `build.toml` (§5.8) applies with no template change — GitLab has no `build_type` matrix axis to narrow. `[filter.options] pybind = false` drops the `Repair Wheel`, `Wheel Deploy` and `Wheel Deploy - Windows` jobs entirely, along with `--wheel-dir` and the in-place Windows repair step; the `Conan Deploy` jobs publish packages rather than wheels and are unaffected.
