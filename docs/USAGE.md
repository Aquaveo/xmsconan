...
2. Sets `XMS_COVERAGE=1` and invokes `build.py` twice in sequence:
   ...
   - Second: `--filter '{"build_type":"Release","options":{"pybind":true,"testing":false,"python_version":"3.13"}}'` — Conan create that builds the library + pybind wheel, then runs `pytest-cov` against the wheel. Not instrumented: `pytest-cov` measures Python lines, and gcovr is pointed at the testing build folder alone. Produces `cov-py.xml`, `cov-py-summary.json`, and `coverage-html-py/` inside the build folder.

   `XMS_COVERAGE=1` rides into the `[buildenv]` of every **non-pybind** profile so CMake adds `--coverage` there. It no longer changes which configurations exist: the testing-only Debug variant is part of the standard packager fan-out either way, and the Python half reuses whatever pybind configuration `[matrix].pybind_build_types` already names. Pybind profiles are left out of the `[buildenv]` injection because gcovr never reads their `.gcda`; the recipe still keys `pytest-cov` off `XMS_COVERAGE` in the inherited process environment, so Python coverage is collected as before. Requiring a Debug pybind build used to cost every dependency a `Debug`+`pybind` binary — the one combination the xms libraries do not publish — and would need a debug interpreter on Windows. `python_version` on the pybind build is pinned to one ABI (highest of `[ci].linux_python_versions` by default, or `[coverage].python_version` when set) so multi-version fan-outs cannot non-deterministically pick whichever pybind config finished last.
