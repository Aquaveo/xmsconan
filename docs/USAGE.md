...
> **Runner / image expectations.** On GitLab a Windows opt-in assumes the `GLR-UV` image can supply the interpreter — it carries `uv` and pre-loaded 3.10 / 3.13 / 3.14, and `uv venv --python` downloads anything else it is asked for, so a version outside that set costs a download on every job rather than failing. A Linux opt-in assumes the matching `conan-gcc13-py<version>` image exists **and is readable by CI** — on GHCR the 3.13 image is public while 3.14 is private, and the generated `container:` block pulls anonymously, so a private image needs its visibility flipped (or a `credentials:` stanza added) before its leg can start.
...
- **Required CI variables with `[ci].windows_vs2019`:** `CONAN_LOGIN_USERNAME_AQUAVEO_VS2019` and
  `CONAN_PASSWORD_AQUAVEO_VS2019`. Conan derives those names by uppercasing the remote name and replacing hyphens
  with underscores, and reads them from the environment — the generated job adds the remote but never logs in
  explicitly, exactly as it does for `aquaveo`.
  **Both the build and the deploy need them — not just the deploy.** `aquaveo-vs2019` is where the msvc 192 build
  *resolves* its third-party stack from, not only where it publishes: the recipe forks to `boost/1.74.0.3` and
  `zlib/1.2.11` on `compiler.version=192`, and those live on that remote. Without credentials conan finds the
  remote needs authentication, drops into an interactive username prompt with no tty, and the job dies while
  computing the dependency graph — before compiling anything — with:

  ```
  ERROR: Package 'boost/1.74.0.3' not resolved: EOF when reading a line. [Remote: aquaveo-vs2019]
  ```

  So set the variables when you set the flag, not before the first release. (An earlier version of this note said
  branch pipelines pass without them and only the tag-time deploy fails. That is wrong: the first branch pipeline
  after opting in fails in every msvc 192 build job.)
...
- **`ERROR: Package 'boost/1.74.0.3' not resolved: EOF when reading a line. [Remote: aquaveo-vs2019]`** — in a msvc 192 *build* job, while computing the dependency graph. The `aquaveo-vs2019` credentials are missing. `EOF when reading a line` is conan dropping into an interactive username prompt with no tty: it found the remote needs authentication, had no `CONAN_LOGIN_USERNAME_AQUAVEO_VS2019` / `CONAN_PASSWORD_AQUAVEO_VS2019` in the environment, and asked. Add both as GitLab CI variables (§10.2). Note that this remote is where the msvc 192 build *resolves* boost and zlib from, not only where it publishes — so the credentials are needed by every msvc 192 pipeline, not just by a tag.
- **`Conan Deploy - Windows VS2019` fails authenticating to `aquaveo-vs2019`.** Same cause and same fix as the entry above; the deploy needs the credentials too.
