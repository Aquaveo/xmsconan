...
| `[filter.options]` | `wchar_t`, `pybind`, `testing`, `python_version` | `pybind` / `testing` take `true` or `false`; `wchar_t` takes `"builtin"` or `"typedef"`; `python_version` takes a **quoted** `"X.Y"` string that appears in `[ci].python_versions` (unquoted `3.13` is a TOML float and is rejected). |
| `[filter.buildenv]` | the names the generated profiles set: `XMS_VERSION`, `PYTHON_TARGET_VERSION`, `CI_COMMIT_TAG`, `RELEASE_PYTHON`, `AQUAPI_*`, `XMS_COVERAGE`, `XMS_TEST_ARTIFACTS_DIR`, `XMS_TEST_ARTIFACTS_LABEL`, `MACOSX_DEPLOYMENT_TARGET`, `_PYTHON_HOST_PLATFORM` | A single value, compared against what the profile sets. |

Everything above is checked by `xmsconan gen` and `xmsconan ci`, including whether the filter as a whole selects anything: a combination no configuration can satisfy (`testing` and `pybind` both `true`, say) fails generation rather than every later build.
...
- **`xmsconan ci`** reads the same table:
  - A pinned `build_type` narrows the `build_type` matrix in every generated GitHub job. GitLab has no `build_type` matrix — its jobs run the whole matrix through `build.py`, which honors the filter anyway.
  - `options.pybind = false` removes the wheel steps (repair, artifact upload, devpi deploy, and `--wheel-dir`) from both pipelines. `xmsconan_wheel_repair` fails on an empty `wheelhouse/`, so a wheel-less library would otherwise get a red pipeline on every branch.
...
- **`xmsconan coverage`** builds Debug + `testing=True` (C++ report) and Debug + `pybind=True` (Python report), pinned to `[coverage].python_version`. Because those two pin the *inverse* of each other, a filter conflicts by requiring an option as much as by excluding it — `pybind = true` cancels the C++ build just as `pybind = false` cancels the Python one. `xmsconan ci` warns about each conflict when `[ci].coverage = true`.
...
- The `build_type` axis defaults to `[Release, Debug]` and shrinks to whatever `[filter].build_type` pins (§5.8), so a Release-only library gets no Debug legs at all. `[filter.options] pybind = false` also drops the wheel repair / upload / deploy steps. An `os`/`arch`/`compiler` pin is *not* applied — those are separate job blocks, so `xmsconan ci` warns about the jobs it would empty instead.
...
- Jobs invoke `build.py` without a `--filter`, so a `[filter]` table in `build.toml` (§5.8) applies with no template change — GitLab has no `build_type` matrix axis to narrow. `[filter.options] pybind = false` drops the `Repair Wheel` and `Wheel Deploy` jobs entirely.
