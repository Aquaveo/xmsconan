...
# 3. Aquaveo modules
from xmsconan._cli import add_verbosity_args, configure_logging, run_main, tracebacks_wanted
...
def _render_context(config: BuildToml, version: str) -> dict:
    ...
    # whatever xmsconan is on the machine, and it is regenerated on
    # every CI run anyway, so it cannot go stale in CI.
