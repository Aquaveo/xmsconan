...
- **Linux and macOS.** `generate()` declares a `VirtualRunEnv`, and pytest runs with `env="conanrun"`, so `LD_LIBRARY_PATH` / `DYLD_LIBRARY_PATH` name every dependency's library directories. Conan degrades gracefully here: if nothing needs a run environment, no `conanrun` script is written and the command runs unwrapped.
