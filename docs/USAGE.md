...
2. Sets `XMS_COVERAGE=1` and invokes `build.py --filter '{"build_type":"Debug","options":{"testing":true,"pybind":true}}'` — one Conan-create that builds the library, links the test runner, and produces the wheel from the *same* `.gcno` set. (`XMS_COVERAGE=1` is also what tells `XmsConanPackager` to emit a `Debug+pybind` configuration in the first place — that combination is normally skipped because the shipping wheel is built `Release`.)
