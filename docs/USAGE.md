...
2. Sets `XMS_COVERAGE=1` and invokes `build.py` twice in sequence:
   ...
   `XMS_COVERAGE=1` is what tells `XmsConanPackager` to emit a `Debug+pybind` configuration (normally Debug+pybind is redundant since the Release wheel is what ships). `python_version` on the pybind build is pinned to one ABI (highest of `[ci].python_versions` by default, or `[coverage].python_version` when set) so multi-version fan-outs cannot non-deterministically pick whichever pybind config finished last.
3. Locates the two build folders in the local Conan cache, runs gcovr against the testing build folder to produce `cov-cpp.xml` and `cov-cpp-summary.json` and `coverage-html-cpp/`, and copies the pytest-cov artifacts out of the pybind build folder.
