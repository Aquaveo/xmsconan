...
The `_d` suffix on the static library comes from `set(CMAKE_DEBUG_POSTFIX _d)`. It does **not** reach the pybind module target on its own: `pybind11_add_module` calls `pybind11_extension`, which sets the target's `DEBUG_POSTFIX` explicitly to `PYTHON_MODULE_DEBUG_POSTFIX` — the `NAME_WE` of the interpreter's `EXT_SUFFIX`, which is the empty string for a release interpreter (`.cp313-win_amd64.pyd`) — and that overrides the directory-scope value. The generated CMakeLists therefore re-asserts `DEBUG_POSTFIX "_d"` on the module target under `if (WIN32 AND NOT PYTHON_MODULE_DEBUG_POSTFIX)`, so the Debug artifacts are `bin/_<name>_d.<abi>.pyd` and `lib/_<name>_d.lib`. The guard keeps a genuine debug Python build, whose postfix is already `_d`, from producing `_<name>_d_d`; and the `WIN32` restriction keeps the module importable off Windows, where there is no import library to link and a Debug pybind build is what coverage instruments.

**A Windows Debug pybind configuration produces no wheel.** Because the postfix is re-asserted there, the Windows Debug module is `_<name>_d.<abi>.pyd` while the generated `_package/pyproject.toml` names the extension module exactly once, as `xms.<dir>._<name>`. A Windows Debug wheel would install a module `import` cannot find, and the Python tests would fail on it; renaming the module is not an option, since consumers link `_<name>_d` by that exact name. So `build()` skips the wheel and the Python tests for **Windows** Debug only — that configuration exists to produce the Conan package a C++ consumer links, and `Release` is what ships to an index. The skip is logged.
