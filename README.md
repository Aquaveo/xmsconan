...
| `xms_python_dependencies` | array[string] | `[]`   | Extra **Python** requirements for `_package/pyproject.toml`, in pip requirement form (`["geopandas", "data_objects>=4.0.0"]`). For runtime imports that are not XMS sister libraries. Wheel metadata only; Conan is unaffected. See `docs/USAGE.md` §5.3.
