...
| `[coverage].python_version` | highest of `[ci].python_versions` (default `"3.13"`) | The single Python ABI the coverage build is pinned to. `xmsconan coverage` builds exactly one `pybind=True+testing=True+Debug` config (the recipe's `run_cxx_tests` and `run_python_tests` both execute against the same instrumented binary so gcovr sees the union of CxxTest- and pytest-driven C++); multi-Python fan-out is intentionally collapsed here so the report is deterministic. Override only when the highest CI version isn't the one you want to gate on. |
...
2. Sets `XMS_COVERAGE=1` and invokes `build.py --filter '{"build_type":"Debug","options":{"pybind":true,"testing":true,"python_version":"3.13"}}'` — one Conan-create that builds the wheel under `--coverage`, runs the CxxTest runner, then runs `pytest-cov` against the wheel. (`XMS_COVERAGE=1` is what tells `XmsConanPackager` to emit the combined `Debug+pybind+testing` configuration in the first place — normally the pybind and testing flags carve independent variants, but coverage merges them so both `run_cxx_tests` and `run_python_tests` execute against the same instrumented binary and contribute `.gcda` data to the same `.gcno` set. Without `testing=True` here, gcovr would only see the pybind-reachable C++ surface, which on most xms libraries is a small slice of the total.) `python_version` is pinned to one ABI (highest of `[ci].python_versions` by default, or `[coverage].python_version` when set — see §5.7) so multi-version fan-outs cannot non-deterministically pick whichever pybind config finished last.
