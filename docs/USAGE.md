...
- **`xmsconan ci`** reads the same table:
  - The `build_type` matrix in every generated GitHub job keeps only the build types that still have configurations. A pinned `build_type` is the obvious case, but so is `[filter.options] pybind = true`: with the default `[matrix].pybind_build_types` of `["Release"]` the Debug leg has nothing to build, and a leg that builds nothing exits `1`. Under `[matrix].wheel_only` the axis a tag pipeline runs is narrower again: only the build types a release still builds on some platform (§10.1). A filter that leaves a release none — `build_type = "Debug"`, which keeps only the Debug testing build — is refused at generation time rather than failing the first tag. GitLab has no `build_type` matrix — its jobs run the whole matrix through `build.py`, which honors the filter anyway.
  ...
  - **`os`, `arch`, and `compiler*` pins are only warned about, not applied.** The platform fan-out lives in separate job blocks rather than a matrix axis, so there is nothing to narrow: `[filter] os = "Windows"` leaves the macOS and Linux jobs to fail with an empty matrix. `xmsconan ci` names each such job at generation time; drop the pin, or turn the job off through `[ci]`. A `compiler*` pin can also empty a job on the **tag pipeline alone**, without emptying it outright: under `[matrix].wheel_only` a GitHub tag drops the testing configurations, so `"compiler.runtime" = "static"` — which leaves msvc nothing else — makes the Windows job's Release leg exit 1 on every release while its branch pipelines pass. The `build_type` axis is one list shared by all four jobs and cannot be narrowed per job either, so that too is named at generation time (§10.1).
...
- **Under `[matrix].wheel_only`, a tag runs only the legs a release still builds.** Every platform job passes `job build --release-skips-testing` (§10.4), which drops the testing configurations on a release, and under `wheel_only` those are the whole Debug half of the matrix, because `[matrix].pybind_build_types` defaults to Release. A Debug leg on a tag therefore matched nothing, and `job build` exited 1 on every platform job of every release while branch pipelines — which resolve `0.0.0`, not a release — stayed green. The `build_type` axis is chosen per pipeline instead: `${{ startsWith(github.ref, 'refs/tags/') && fromJSON('["Release"]') || fromJSON('["Release", "Debug"]') }}`, with the two lists also spelled out in a comment above the workflow's `name:`. A branch pipeline keeps its Debug leg, so the Debug C++ suite still runs on every push; a tag leaves it out rather than running it empty, which is the line GitLab already draws by keeping every testing build off a tag. The tag list is measured from the matrix, not assumed: a Debug wheel is not a testing configuration, so a library naming `Debug` in `pybind_build_types` keeps that leg and gets the plain list. The list is one for all four jobs, so it is a union over platforms: a leg only one platform empties stays on a tag and still fails there — under `wheel_only`, `[filter] "compiler.runtime" = "static"` leaves Windows, whose pybind build is dynamic-only, nothing but testing builds, so its Release leg exits 1 on a tag. `xmsconan ci` warns about exactly that at generation time, naming the job and the leg — it measures the release legs per platform as well as unioned, and reports each emitted job the tag axis would run somewhere it builds nothing. It is a warning rather than a refusal because the rest of the workflow is still correct: the platforms that do build the leg are unaffected, and the same filter is how a repository deliberately narrows one platform. A job the filter empties *outright* is left to the empty-matrix warning above, which names the pin responsible instead of repeating it once per leg. Without `wheel_only` nothing is dropped. `xmsconan ci` refuses a `[filter]` that would leave a release no leg at all — `build_type = "Debug"` under `wheel_only` keeps only the Debug testing build. `job build`'s own empty-match exit is unchanged: a leg that matches nothing is still an error.
...
| `--release-skips-testing` | off | On a release version, drop the testing configurations. This is the GitLab Windows job's old tag rule, and, under `[matrix].wheel_only`, every GitHub platform job's: nothing installs a test runner and no tag pipeline runs one. On GitHub it also replaced a JSON `--filter` the template selected through a step-level `env:` expression, and on the Windows leg through a second `if:`-gated step -- `shell: cmd` expands `%VAR%` before the argument is parsed, so that leg could not hold the filter in a variable. The tool tests the same tag the expressions did, from the version it already resolved (§10). A flag rather than an unconditional rule, because the jobs that loop a whole matrix publish from that same build and their tarball would lose binaries a release ships. Under `wheel_only` on GitHub the generator also leaves out of a tag pipeline's `build_type` axis every leg this flag would empty on every platform — the Debug leg, unless `[matrix].pybind_build_types` names Debug (§10.1) — rather than letting it run and exit 1 on an empty match. |
