...
- **msvc 194 binaries turned up on the `aquaveo-vs2019` remote.** Something published without `--package-query compiler.version=192`. Both `conan cache save` and `conan upload` match by reference, and a runner's Conan cache is shared across the jobs on that machine, so an unqueried publish from the msvc 192 job carries whatever the msvc 194 job left behind. The generated pipeline passes the query on every deploy step; a hand-run `xmsconan_conan_deploy` or a `build.py --upload` without `--platform windows_vs2019` does not.
