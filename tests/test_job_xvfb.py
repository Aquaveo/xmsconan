...
from .job_helpers import build_toml_config
from .utils import patch_env

#: What every case here needs before xvfb is even considered.
XVFB_BODY = 'library_name = "xmscore"\n[ci]\nxvfb = true\n'
...
def test_ensure_socket_dir_creates_the_directory_idempotently(tmp_path):
    """A fresh container has no /tmp/.X11-unix; make one, twice."""
    target = tmp_path / "x11-sockets"

    with patch.object(xvfb, "SOCKET_DIR", str(target)):
        xvfb.ensure_socket_dir()
        # A second call must be a no-op, not an error: concurrent cold-starting
        # servers all call it, and the loser of the race must not fail.
        xvfb.ensure_socket_dir()
...
def test_ensure_socket_dir_makes_it_sticky_and_world_writable(tmp_path):
    """The mode X expects, so every shard's server can bind its own socket.

    Split from the creation case rather than guarded in its body: the mkdir is
    worth checking on Windows too, and an in-body ``if`` there reports a pass
    for a case it skipped.
    """
    target = tmp_path / "x11-sockets"
...
def test_server_answers_requires_a_success_reply():
    ...
    """
    # A short mkdtemp, not tmp_path: an AF_UNIX path is capped near 104 bytes
    # and pytest's directory names are long. Patched in rather than the real
    # /tmp/.X11-unix, which two CI jobs on one host would collide in.
    socket_dir = tempfile.mkdtemp(prefix="xa-")

    def _serve(display_number, reply):
        path = f"{socket_dir}/X{display_number}"
    ...
    try:
        _serve(47901, struct.pack("<B", 1))
        _serve(47902, struct.pack("<B", 0))
        with patch.object(xvfb, "SOCKET_DIR", socket_dir):
...
def test_stop_server_escalates_to_kill():
...
def test_display_is_a_no_op_when_no_display_is_wanted(tmp_path):
    ...
    environ = {}
    with xvfb.display(build_toml_config(tmp_path, XVFB_BODY), environ=environ, platform="darwin") as server:
...
def test_display_exports_display_and_stops_the_server(mock_which, tmp_path):
    ...
    with patch.object(xvfb, "ensure_socket_dir", lambda: None), \
        ...
        with xvfb.display(build_toml_config(tmp_path, XVFB_BODY), log_dir=tmp_path, environ=environ,
                          platform="linux"):
...
def test_display_raises_when_the_server_never_answers(mock_which, tmp_path):
    ...
    with patch.object(xvfb, "ensure_socket_dir", lambda: None), \
        ...
        with pytest.raises(RuntimeError, match="did not answer"):
            with xvfb.display(build_toml_config(tmp_path, XVFB_BODY), log_dir=tmp_path, environ={},
                              platform="linux"):
