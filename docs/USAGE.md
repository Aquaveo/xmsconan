...
| `[ci].test_shards` | `0` | When >1, run the C++ tests as N gtest shards **inside a single container**. The recipe skips `cmake.test()` in this mode, so the shards are the only thing that runs the C++ tests — a missing runner binary fails the build rather than being skipped. On GitLab it needs `split_tests = true`, and each `Run C++ Tests - <label>` job runs `xmsconan job test`, which reads the count from `build.toml`, forks N runner processes and merges their JUnit XML into one `TEST-cxxtest.xml` (§10.2). The count is not rendered into the pipeline, so raising it does not require regenerating the CI. The shard count is per job, so a two-configuration matrix runs `2 × N` processes across two containers. On GitHub, where the build job runs its own tests, the same `xmsconan job build` reads the count and shards in place -- the flag is rendered into neither pipeline now, so raising it is a `build.toml` edit on either host. This was previously GitLab's `parallel: N`, which bought its concurrency by starting N containers — each one repeating the container start, the `pip install` and the artifact download to run one Nth of the suite. |
