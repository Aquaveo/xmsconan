...
| `[ci].test_shards` | `0` | When >1, run the C++ tests as N gtest shards **inside a single container**. The recipe skips `cmake.test()` in this mode, so the shards are the only thing that runs the C++ tests — a missing runner binary fails the build rather than being skipped. On GitLab it needs `split_tests = true`, and each `Run C++ Tests - <label>` job runs `xmsconan job test`, which reads the count from `build.toml`, forks N runner processes and merges their JUnit XML into one `TEST-cxxtest.xml` (§10.2). The count is not rendered into the pipeline, so raising it does not require regenerating the CI. The shard count is per job, so a two-configuration matrix runs `2 × N` processes across two containers. On GitHub, where the build job runs its own tests, it reaches `build.py --test-shards N` in every platform job instead. This was previously GitLab's `parallel: N`, which bought its concurrency by starting N containers — each one repeating the container start, the `pip install` and the artifact download to run one Nth of the suite. |
...
- **CI matrices expand.** GitHub Actions: `python-version` on the mac, linux,
  ...
  over `PYTHON_TARGET_VERSION` on `Conan Build` (from `linux_python_versions`)
  and on `Conan Build - Windows` (from `python_versions`) — plus
  `Conan Build - Windows VS2019` when `[ci].windows_vs2019` is set, over the same
  list. The three `Conan Deploy` jobs do not fan out: they restore and upload,
  which is ABI-independent, and `dependencies:` hands one instance every tarball
  the fan-out produced (§10.2). On Windows the variable is
...
2. **Run the job** -- one `xmsconan job <kind>` call (§10.4). On GitLab that is the whole of the script: `job build` sets up Conan, generates the build files, builds this job's leg and stages its wheel; `job test`, `job package`, `job deploy`, `job lint` and `job coverage --pages` are the rest. On GitHub the steps are still spelled out individually — see §10.1.
3. **On tag pushes:** `xmsconan job deploy` -- the wheel upload, and the Conan upload of every tarball the build jobs left in `.export/` -- on GitLab, `python build.py --skip-build --upload` in the build job on GitHub
...
- Required CI variables: `AQUAPI_URL`, `AQUAPI_USERNAME`, `AQUAPI_PASSWORD` (for wheel deploy). Set the username
  ...
  They are read from the process environment by the wheel half of `xmsconan job deploy`, which hands them to `uv publish` through
...
Every GitLab job's script is now an install line and one `xmsconan job` call. The six subcommands are the six things a pipeline does:

| Command | Job it runs | What it derives |
|---|---|---|
| `xmsconan job build` | `Conan Build`, the per-leg Linux jobs, both Windows jobs | Conan setup, `xmsconan gen` at the resolved version, the configuration matrix and its filters, the wheel, the export tarball |
| `xmsconan job test` | `Run C++ Tests - <label>` | Shard count, artifacts directory, `[ci].xvfb`, the JUnit report name |
| `xmsconan job package` | `Repair Wheel` | The wheel directory to repair |
| `xmsconan job deploy` | `Wheel Deploy`, `Wheel Deploy - Windows`, all three `Conan Deploy` jobs | Which tarballs `.export/` holds, the remote and package query for this platform, the wheel directory, the devpi credentials |
| `xmsconan job lint` | `Lint` | `xmsconan gen` at the pinned fallback version, then `flake8 _package` |
...
| `--cache-archive PATH` | off | After uploading, write `conan cache save` to `PATH`, restricted to the same query as the upload. For a job that wants the published binaries as a file — a GitHub release asset, or a hand-run investigation. |
| `--version` / `--toml` | resolved (§10), `build.toml` | As everywhere else. |
...
Three environment variables the template used to `export` are set by `job build` instead, each only when the environment has not already chosen a value — so a bigger runner can still raise a level without regenerating the CI:
